...
static inline void string_append(String *s,char c){
    if (s->nalloc==(s->len+1)){ //if the string is full, realloc the string
        realloc_body(s);
    }
    s->body[s->len++]=c;
    s->body[s->len]='\0';
...
//fmt: format string,like "%s %d"
static inline void string_appendf(String *s,char *fmt, ...){
    va_list args;
    while (1){
        int avail=s->nalloc-s->len; //available space

        va_start(args,fmt);
        //append the formatted string to the s->body last position
        int written=vsnprintf(s->body+s->len,avail,fmt,args);
        va_end(args);

        //if ava space is not enough, realloc s->body and rewrite
        if (avail<=written){
            realloc_body(s);
            continue;
        }
