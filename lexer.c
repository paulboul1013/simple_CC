...
/* skip space, newline, and carriage return */
...
//receive a number or float number
static Token read_number(char c){
    String s=make_string();
    string_append(&s,c);
    while (1) {
        int c=getc(stdin); //read next character
...
static Token read_char(void){
    ...
    if (c2!='\''){
        error("wrong format of char literal");
    }
    return make_char(c);
err:
